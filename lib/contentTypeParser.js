...
ContentTypeParser.prototype.add = function (contentType, opts, parserFn) {
  ...
  if (contentTypeIsString && contentType === '*') {
    ...
    if (contentTypeIsString) {
      ...
      var normalizedContentType = ct.toString()
...
ContentTypeParser.prototype.existingParser = function (contentType) {
  if (typeof contentType === 'string') {
    var ct = new ContentType(contentType).toString()
...
ContentTypeParser.prototype.getParser = function (contentType) {
  ...
  for (var j = 0; j !== this.parserRegExpList.length; ++j) {
    var parserRegExp = this.parserRegExpList[j]
    var matchIdx = ct.search(parserRegExp)
    if (matchIdx !== -1 && matchIdx < contentType.mediaType.length) {
...
ContentTypeParser.prototype.remove = function (contentType) {
  ...
  var removed = this.customParsers.delete(contentType)

  var idx = -1
  for (var i = 0; i < parsers.length; i++) {
    if (parsers[i].toString() === contentType) {
      idx = i
      break
    }
  }
...
function compareContentType (contentType, parserListItem) {
  if (parserListItem.isEssence) {
    ...
    for (var pi = 0; pi < parserListItem.parameterKeys.length; pi++) {
      var key = parserListItem.parameterKeys[pi]
